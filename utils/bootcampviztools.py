...
def plot_grouped_histograms(df, cat_col, num_col, group_size, bins = "auto", dict_colors = {}):
    unique_cats = df[cat_col].unique()
    num_cats = len(unique_cats)
    ...
    for i in range(0, num_cats, group_size):
        ...
        for cat in subset_cats:
            if cat in dict_colors.keys():
                sns.histplot(subset_df[subset_df[cat_col] == cat][num_col], kde=True, label=str(cat), bins = bins, color=dict_colors[cat])
            else:
...
def grafico_dispersion_con_correlacion(df, columna_x, columna_y, tamano_puntos=50, mostrar_correlacion=False):
    ...
    if mostrar_correlacion:
        ...
        plt.title(f'Diagrama de Dispersión ({columna_x}-{columna_y}) con Correlación: {correlacion:.2f}')
    else:
